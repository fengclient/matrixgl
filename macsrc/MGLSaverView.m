...
- (void) readDefaults:(ScreenSaverDefaults *) inDefaults
{
	...
	tObject=[inDefaults objectForKey:@"Credits"];
	
	if (tObject==nil || [inDefaults boolForKey:@"Credits"]==YES)
	{
		settings_->classic=0;
		settings_->pic_offset=(num_pics+1)*(rtext_x*text_y);
		settings_->pic_mode=1;
...
- (void) writeDefaults
{
	...
	[tDefaults setInteger:[IBshowCredits_ state] forKey:@"Credits"];
...
- (void) setDialogValue
{
	...
	tObject=[tDefaults objectForKey:@"Credits"];
	
	if (tObject==nil || [tDefaults boolForKey:@"Credits"]==YES)
	{
		[IBshowCredits_ setState:NSOnState];
	}
	else
	{
		[IBshowCredits_ setState:NSOffState];
	}
	...
	if ([IBshowImages_ state]==NSOffState)
	{
		[IBshowCredits_ setState:NSOffState];
		
		[IBshowCredits_ setEnabled:NO];
	}
	else
	{
		[IBshowCredits_ setEnabled:YES];
	}
